import { getSupabaseAdmin, handleError, json, nextRevision, readJsonBody, requireAccountContext, requireEstimatorAssignment } from "./_shared.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const record of incoming) {
      ...
      const status = record.estimator?.status || "assigned";
      const values = { company_id: context.companyId, external_job_id: record.id, owner_user_id: context.user.id, schema_version: Number(record.estimator?.schemaVersion || 2), status, record_data: record, revision, updated_at: new Date().toISOString() };
      let write;
