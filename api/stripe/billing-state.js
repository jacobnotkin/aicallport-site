...
function parseMonthlyCallVolume(value) {
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "POST") {
      ...
      if (action === "submit-beta-application") {
        ...
        const feedbackCommitment = typeof body.feedbackCommitment === "string" ? body.feedbackCommitment.trim() : "";
        const validFeedbackCommitments = ["Yes", "No"];

        if (!fullName || !companyName || !email || !phone || !businessType || !volume || !problem || !feedbackCommitment) {
          return json(res, 400, { error: "All founder beta request fields are required." });
        }

        if (!validFeedbackCommitments.includes(feedbackCommitment)) {
          return json(res, 400, { error: "Invalid feedback commitment value." });
        }
        ...
        const { data: existingApplication } = await supabaseAdmin
          ...
          .in("status", ["new", "reviewing", "shortlisted", "approved", "contacted"])
        ...
        if (existingApplication?.id) {
          return json(res, 409, {
            error: "A founder beta request for this company and email is already in review."
          });
        }

        const betaApplicationPayload = {
          full_name: fullName,
          company_name: companyName,
          email,
          phone,
          business_type: businessType,
          monthly_inbound_calls_or_leads: volume,
          biggest_problem_today: problem,
          willing_to_provide_feedback: feedbackCommitment,
          status: "new",
          activation_stage: "not_started",
          review_notes: ""
        };

        const { data: betaApplication, error: betaApplicationError } = await supabaseAdmin
          .from("beta_applications")
          .insert(betaApplicationPayload)
          .select("*")
          .single();

        if (betaApplicationError || !betaApplication) {
          return json(res, 500, { error: betaApplicationError?.message || "Unable to submit founder beta request." });
        }
        ...
        const leadPayload = {
          ...
          monthly_call_volume: parseMonthlyCallVolume(volume),
          next_action: "Review founder beta request and assign next action.",
          notes: `Founder beta request submitted from prelaunch page. Feedback commitment: ${feedbackCommitment}. Volume bucket: ${volume}.`
        };
        ...
        if (leadError || !lead) {
          return json(res, 500, { error: leadError?.message || "Founder beta request was saved, but lead creation failed." });
        }

        return json(res, 200, {
          request: betaApplication,
          lead
        });
      }
    ...
    if (view === "leads") {
      const { data, error } = await supabaseAdmin
        .from("leads")
        .select("*")
        .or("beta_application_id.not.is.null,converted_company_id.not.is.null")
        .order("created_at", { ascending: false });
